...
const _favoriteSongScript = r'''
  (function () {
    var payload = window.__EMOC_FAVORITE__ || {};
    var songId = String(payload.songId || '').trim();
    var playlistId = String(payload.playlistId || '').trim();
    var playlistTitle = payload.playlistTitle || '';
    var playlistKind = payload.playlistKind || '';
    ...
    function favoriteButton() {
      ...
      return bar.querySelector('.icn-add.j-flag,.icn-add,.u-icn-add,.oper .icn-add,[data-action="fav"],[data-action="like"]') ||
        document.querySelector('.icn-add.j-flag,.icn-add,.u-icn-add,[data-action="fav"],[data-action="like"]');
    }
    function clickPlaylistInPopup() {
      var selectors = [
        '[data-id="' + playlistId + '"]',
        '[data-res-id="' + playlistId + '"]',
        'a[href*="playlist?id=' + playlistId + '"]'
      ];
      for (var s = 0; s < selectors.length; s++) {
        var direct = document.querySelector(selectors[s]);
        if (direct && click(direct.closest('li,.item,.f-cb') || direct)) return true;
      }
      var nodes = Array.prototype.slice.call(document.querySelectorAll('.m-layer li,.m-layer .item,.lyct li,.lyct .item,li,.item'));
      for (var i = 0; i < nodes.length; i++) {
        var text = clean(nodes[i].textContent);
        if (playlistTitle && text.indexOf(playlistTitle) >= 0) {
          if (click(nodes[i])) return true;
        }
      }
      return false;
    }
    ...
    function playlistApiRequest(url, body) {
      return fetch(url, {
        method: 'POST',
        credentials: 'include',
        ...
        .then(function (data) {
          ...
          throw new Error(data.message || data.msg || code || '接口未确认成功');
        });
    }
    function addToPlaylistApi() {
      var token = csrf();
      var trackIds = JSON.stringify([Number(songId)]);
      var body = 'op=add' +
        '&pid=' + encodeURIComponent(playlistId) +
        '&trackIds=' + encodeURIComponent(trackIds) +
        '&tracks=' + encodeURIComponent(songId) +
        '&imme=true' +
      ...
      var query = '?op=add' +
        ...
        '&csrf_token=' + encodeURIComponent(token);
      var attempts = [
        ['/api/playlist/manipulate/tracks', body],
        ['/api/playlist/manipulate/tracks' + query, 'csrf_token=' + encodeURIComponent(token)]
      ];
      return attempts.reduce(function (chain, attempt) {
        return chain.catch(function () {
          return playlistApiRequest(attempt[0], attempt[1]);
        });
      }, Promise.reject(new Error('start')))
        .then(function () {
          post('已收藏到：' + playlistTitle);
        })
        .catch(function (error) {
          post('收藏失败：' + error.message);
        });
    }
    if (!songId || !playlistId) {
      post('当前歌曲或歌单 ID 不完整，无法收藏');
      return;
    }
    if (playlistKind !== 'liked') {
      addToPlaylistApi();
      return;
    }
    click(favoriteButton());
    setTimeout(function () {
      if (clickPlaylistInPopup()) {
        post('已点击官网收藏：' + playlistTitle);
        return;
      }
      addToPlaylistApi();
    }, 600);
...
String _playerControlScript(String action, {double? value}) {
  ...
  (function () {
    ...
    if (action === 'favorite') click(pick('.icn-add.j-flag,.icn-add,.u-icn-add,.oper .icn-add,[data-action="like"],[data-action="fav"]'));
    if (action === 'mode') {
